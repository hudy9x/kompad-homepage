...
export default function Accessibility() {
  ...
  return (
    <div
      id="accessibility"
      className="pt-[100px] sm:pt-[120px] py-[115px] border-b border-gray-200 bg-white relative"
    >
      <div
        className="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none"
        style={{
          backgroundImage:
            "url('vecteezy_office-and-school-stationery-seamless-background-pattern_.jpg')",
          backgroundSize: "350px",
        }}
      ></div>
      <div className="main-box">
        <h2 className="title2 text-gray-700">Accessibility</h2>
        <p className="text-center pt-6 px-6 sm:px-0 text-gray-500 uppercase italic font-sans tracking-widest">{`/* You focus on the content, Kompad helps with the rest */`}</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-[50px] sm:w-[1020px] m-auto text-center">
          {accessList.map((item, index) => {
            ...
            return (
              <div className="access-item mb-8 basis-72" key={index}>
                <Image
                  src={item.src}
                  alt="Image"
                  height={200}
                ...
                />
                <div
                  className={`access-title text-xl py-2 pt-4 font-extrabold text-gray-800`}
                >
                  <span className={colors[index]}>#{index + 1} </span>
                  {item.title}
                </div>
                <p className="text-gray-500 text-sm px-8 sm:px-0">
                  {item.desc}
                </p>
              </div>
            );
